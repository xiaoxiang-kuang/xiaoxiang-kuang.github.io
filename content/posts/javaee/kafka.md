...
* Log对应的命名形式为`topic-partition`的目录。
* 为了便于消息的检索，每隔LogSegment中的日志文件（以`.log`文件结尾）都有对应的两个索引文件：偏移量索引文件（`.index`）和时间戳索引文件(`.timeindex`)，每个logsegment都有一个基准偏移量baseoffset，用来表示当前logsegment中第一条消息的offset，偏移量是一个64位长整型数，日志文件和两个索引文件都是根据准偏移量来命名的，名称固定为20个数字，没有达到的位数则用0填充。
* kafka消费者的唯一是保存在内部topic `__consumer_offsets`中的。
